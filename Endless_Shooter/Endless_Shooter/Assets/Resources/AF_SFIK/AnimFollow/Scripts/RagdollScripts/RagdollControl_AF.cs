...
namespace AnimFollow
{
	...
	//	 This kind of a state machine that takes the character through the states: colliding, falling, matching the masters pose and getting back up
		...
		[Range(0f, 10f)] public float contactForce = 1f;
		[Range(0f, 30f)] public float contactJointTorque = 3f;
		...
		void Awake () // Initialize
		{
			...
			anim.speed = animatorSpeed; // set the animator speed to the setting in player movement. RagdollControl varies the animator speed, best to not set animator speed anywhere else
			...
			foreach(Rigidbody slaveRigidBody in slaveRigidBodies) // Distribute limb script
				slaveRigidBody.gameObject.AddComponent<AnimFollow.Limb_AF>(); // Destribute a collision scripts to all limbs. This script reports to RagdollControll if any limb is in contact with another collider
	...
	//		Debug.DrawRay(ragdollRootBone.position, ragdollRootBone.rotation * rootboneToForward * Vector3.forward); // Open this and check that the ray is pointing as the nose of the charater
			...
			if (shotByBullet || numberOfCollisions > 0 && (collisionSpeed > graceSpeed || (!(gettingUp || falling) && limbErrorMagnitude > noGhostLimit)) || (limbErrorMagnitude > noGhostLimit2 && orientated))
			{
				if (!falling)
				{
					...
					if (!anim.GetCurrentAnimatorStateInfo(0).nameHash.Equals(hash.idle) && ! getupState) // If not in idle state
					{
			...
			{	
				if (gettingUp)
				{
					...
					if (orientated)
					{
						if (animFollow.maxTorque < wakeUpStrength) // Ease the ragdoll to the master pose. WakeUpStrength limit should be set so that the radoll just has reached the master pose
						{
							...
							anim.speed = getup1AnimatorSpeedFactor * animatorSpeed; // Slow the animation briefly to give the ragdoll time to ease to master pose
						...
						{
							anim.speed = getup2AnimatorSpeedFactor * animatorSpeed; // Animation speed during get up state
				...
				else // Falling
				{
					...
					if (ragdollRootBone.GetComponent<Rigidbody>().velocity.magnitude < settledSpeed) // && contactTime + noContactTime > .4f)
					{
						...
						anim.speed = masterFallAnimatorSpeedFactor * animatorSpeed; // Animation speed during transition to get up state
						...
						anim.SetFloat(hash.speedFloat, 0f, 0f, Time.fixedDeltaTime);
						...
						{
							if (!anim.GetCurrentAnimatorStateInfo(0).nameHash.Equals(hash.getupFront))
								anim.SetBool(hash.frontTrigger, true);
							else // if (!anim.GetCurrentAnimatorStateInfo(0).IsName("GetupFrontMirror"))
								anim.SetBool(hash.frontMirrorTrigger, true);
						}
						else
						{
							if (!anim.GetCurrentAnimatorStateInfo(0).nameHash.Equals(hash.getupBack))
								anim.SetBool(hash.backTrigger, true);
							else // if (!anim.GetCurrentAnimatorStateInfo(0).IsName("GetupFrontMirror"))
								anim.SetBool(hash.backMirrorTrigger, true);
						}
					}
				}
			}
		...
		bool WeHaveAllTheStuff()
		{
			...
			if (!(anim = master.GetComponent<Animator>()))
			{
