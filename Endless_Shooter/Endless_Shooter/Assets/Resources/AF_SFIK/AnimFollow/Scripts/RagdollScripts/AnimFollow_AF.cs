...
#region
		...
		Transform[] masterTransforms; // These are all auto assigned
		Transform[] masterRigidTransforms = new Transform[1];
		Transform[] slaveTransforms;
		Rigidbody[] slaveRigidbodies = new Rigidbody[1];
	...
	//	public float[] PTorqueProfile = {20f, 30f, 10f, 30f, 10f, 30f, 30f, 30f, 10f, 30f, 10f}; // Per limb world space torque strength
		...
		// The ranges are not set in stone. Feel free to extend the ranges
		[Range(0f, 48f)] public float angularDrag = 1f; // Rigidbodies angular drag. Unitys parameter
		...
		bool torque = true; // Use World torque to controll the ragdoll
		bool force = true; // Use force to controll the ragdoll
	...
	//		Debug.Log("The script AnimFollow has set the fixedDeltaTime to " + fixedDeltaTime); // Remove this line if you don't need the "heads up"
		...
		public void DoAnimFollow()
		{
			...
			for (int i = 0; i < slaveRigidTransforms.Length; i++) // Do for all rigid bodies
			{
				...
				forceError = masterRigidTransforms[i].position - slaveRigidTransforms[i].position; // + new Vector3(2f, 0f, 0f);
	...
	//				slaveRigdbodies[i].AddForce(forceSignal, ForceMode.VelocityChange); // Try this or the line below and decide which you like better
					slaveRigidbodies[i].AddForceAtPosition(forceSignal, slaveRigidTransforms[i].position, ForceMode.VelocityChange);
				}
